(ns vercel.main
  "Kotodama WASM entrypoint for the Vercel clean-room actor (L4) — Clojure port.
  ...
  facts (`vercel.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "vercel")
(def tier "L4")
...
(def entity-specs
  [{:entity "Deployment" :plural "deployments" :id-prefix "vercel_dep"
    :fields [:name :projectId :url :readyState :target :type] :required [:name :url]
    :coerce {}                       :refs {:projectId "Project"}}
   {:entity "Project"    :plural "projects"    :id-prefix "vercel_pro"
    :fields [:name :accountId :nodeVersion :framework :buildCommand] :required [:name :nodeVersion]
    :coerce {}                       :refs {}}
   {:entity "Domain"     :plural "domains"     :id-prefix "vercel_dom"
    :fields [:name :verified :serviceType :expiresAt] :required [:name :verified]
    :coerce {:verified :bool :expiresAt :int} :refs {}}
   {:entity "Alias"      :plural "aliases"     :id-prefix "vercel_ali"
    :fields [:alias :deploymentId :projectId :redirectStatusCode] :required [:alias :redirectStatusCode]
    :coerce {:redirectStatusCode :int} :refs {:deploymentId "Deployment" :projectId "Project"}}
   {:entity "Team"       :plural "teams"       :id-prefix "vercel_tea"
    :fields [:name :slug :description]        :required [:name :slug]
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"vercel.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "vercel-compat" :tier tier :entities entities} 200])
...
(defn start! [] :vercel-compat/ready)
